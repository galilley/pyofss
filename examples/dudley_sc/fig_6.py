...
system.add(Fibre(length=length, gamma=gamma, beta=beta_full[:3], self_steepening=False, use_all='hollenbeck',
...
map_plot(x, dB(y/np.max(y)), z, labels["nu"], labels["P_nu"], z_label,
         filename="fig_6_map_nu", y_range=(-40.0, 0))

single_plot(nu_to_lambda(x), y[-1], labels["Lambda"], labels["P_lambda"],
         filename="fig_6_lambda", use_fill=False)

(x, y, z_temp) = storage.get_plot_data(reduced_range=(-1, 2))
z = z_temp * 1.0e3

map_plot(x, dB(y/np.max(y)), z, labels["t"], labels["P_t"], z_label,
        filename="fig_6_map_t", y_range=(-40.0, 0))

single_plot(x, y[-1], labels["t"], labels["P_lambda"],
         filename="fig_6_t", use_fill=False)
